...
public class DropDownListHelp
{
    ...
    private void addChildItems(string parentKeyField, string parentKey, string keyField, string textField, string sortString, DropDownList ddl)
    {
        ...
        int a = dv.Count;
        bool isLast= dt.Columns.IndexOf("islast")==-1?false:true;
        ...
        for (int i = 0; i < a; i++)
        {
            ...
            dv.RowFilter = parentKeyField + "='" + parentKey + "'";
            dv.Sort = sortString;
            ListItem item=new ListItem(gridline + (i == a - 1 ? "┗" : "┣") + dv[i][textField].ToString(), dv[i][keyField].ToString());
            ddl.Items.Add(item);
            if (isLast)
            {
               item.Attributes.Add("islast",dv[i]["islast"].ToString());
            }
