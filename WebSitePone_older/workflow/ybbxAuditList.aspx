<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ybbxAuditList.aspx.cs" Inherits="bxd_ybbxAuditList" EnableViewState="false" %>
...
<head id="Head1" runat="server">
    ...
    <script>
        ...
        function View(flowId, code) {
            ...
            $.post("GetBillType.ashx", { "billcode": code, "isdz": "1" }, function (data, status) {
                if (status == "success") {
        ...
        function Audit(flowId, code, obj) {
            if (confirm("是否确定审核？")) {
                ...
                // alert(mind);
                $.post("WorkFlowApprove.ashx", { "billcode": code, "mind": mind, "action": "approve", "isdz": "1" }, function (data, status) {
        ...
        function AuditChildren() {
            if (confirm("是否确定审核？")) {
                ...
                $.post("../workflow/WorkFlowApprove.ashx", { "billcode": code, "mind": mind, "action": "approve", "isdz": "1" }, function (data, status) {
                    if (data < 0 && status != "success") {
                        alert("审核失败！");
                    }
                    else {
        ...
        function Cancel(flowId, code, obj) {
            if (confirm("是否确定驳回？")) {
                var mind = $(obj).siblings("span").find("input").val();
                $.post("WorkFlowApprove.ashx", { "billcode": code, "mind": mind, "action": "disagree", "isdz": "1" }, function (data, status) {
                    if (data < 0 && status != "success") {
                        alert("驳回失败！");
                    }
                    else {
        ...
        function CancelChildren() {
            if (confirm("是否确定驳回?")) {
                ...
                $.post("../workflow/WorkFlowApprove.ashx", { "billcode": code, "mind": mind, "action": "disagree", "isdz": "1" }, function (data, status) {
                    if (data < 0 && status != "success") {
                        alert("驳回失败！");
                    }
                    else {
    ...
    <form id="form1" runat="server" data-role="page" data-theme="b" method="post" data-ajax="false">
        <div>
            <div data-role="header">
                <a href="Audit.aspx?isdz=1" data-icon="back" data-ajax="false">返回</a>
                <h1>
                    ...
                    <ItemTemplate>
                        <div data-role="collapsible" data-collapsed="false" data-theme="b" class="c-main">
                            ...
                            <div class="c_row_item">
                                ...
                                <asp:HiddenField ID="hiddatetime" runat="server" Value='<%#Eval("tbilldate") %>' />
                            ...
                            <div class="c-option">
                                ...
                                <a data-role='button' data-inline='true' title='<%#Eval("billname") %>' onclick="Audit('<%#Eval("flowID") %>','<%#Eval("billname") %>',this)"
                                    data-theme='d'>
                                    <img src='../images/metro/Check.png' />通过</a> <a data-role="button" data-inline="true"
                                        title='<%#Eval("billname") %>' onclick="Cancel('<%#Eval("flowID") %>','<%#Eval("billname") %>',this)"
                                        data-theme="d">
