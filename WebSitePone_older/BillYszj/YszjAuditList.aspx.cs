...
public partial class BillYszj_YszjAuditList : System.Web.UI.Page
{
    ...
    private void BindData()
    {
        ...
        DataTable dt = PubMethod.GetPageData(sql, "YszjAuditList.aspx", out pageNav);
    ...
    private string GetSql()
    {
        ...
        if (list.Count > 0)
        {
            sb.Append(" select  *,(select top 1 (xmmc) from bill_ysgc a,bill_ysmxb  b      where a.gcbh=b.gcbh and b.billcode=bill_main.billcode  ) as ysgc ,  Row_number()over(order by billdate desc,billName desc) as crow from bill_main  where  flowid='" + flowid + "' and  billcode in(");
