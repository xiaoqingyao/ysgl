    ...
    protected void btn_select_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < myGrid.Items.Count; i++)
        {
            ...
            if (cbox.Checked == true)
            {
                ...
                sqls.Add("insert into bill_qtbxmxb_fykm_dept select '" + Page.Request.QueryString["mxGuid"].ToString().Trim() + "','" + guid2 + "',deptcode,0,0 from bill_departments where deptcode='" + myGrid.Items[i].Cells[1].Text + "' and deptcode not in (select deptcode from bill_qtbxmxb_fykm_dept where kmmxguid='" + Page.Request.QueryString["mxGuid"].ToString().Trim() + "')");
                sel_count++;
            }
        }
        if (selectLev3 == false)
