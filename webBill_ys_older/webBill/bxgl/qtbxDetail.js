    function refreshInfo()
    {
        ...
        var type=document.getElementById("lblType").innerHTML;
            var info=webBill_bxgl_qtbxDetail.bindJsInfoJson(billCode,type).value;
...
//            $("#lblYbje").html(YtybArr[1]);
    ...
    function selectry(openUrl)
    {
        ...
        { 
            var dept = webBill_bxgl_qtbxDetail.getUserInfo(str).value;
    ...
    {
        var returnValue=webBill_bxgl_qtbxDetail.DeleteBxdj(djGuid).value;
        if(returnValue==true)
        {
    ...
    {
        var str=window.showModalDialog('qtbxselectYskm.aspx?sqrq='+sqrq+'&deptCode='+deptCode+'&billCode='+billCode+'', 'newwindow', 'center:yes;dialogHeight:520px;dialogWidth:300px;status:no;scroll:yes');
        if(str!=undefined&&str!="")
        { 
    ...
    {
        var str=window.showModalDialog('qtbxselectYskm.aspx?billCode='+billCode+'', 'newwindow', 'center:yes;dialogHeight:520px;dialogWidth:300px;status:no;scroll:yes');
        if(str!=undefined&&str!="")
        { 
            document.getElementById("btnRefresh").click();//刷新数据
        }
    ...
    function DeleteYskm()
    {
        ...
        var returnValue=webBill_bxgl_qtbxDetail.DeleteFykmMx(fykmGuid,billCode).value;
        if(returnValue==true)
        {
            calYtYb();//计算应退应补
            document.getElementById("btnRefresh").click();//刷新数据
    ...
    {
        var billCode=document.getElementById("lblBillCode").innerHTML;
        var returnValue=webBill_bxgl_qtbxDetail.AddFtMxb(billCode,kmmxGuid).value;
        if(returnValue!="")
        { 
            alert(returnValue);
        }
    ...
    function DeleteFT(mxGuid)
    {
        ...
        var returnValue=webBill_bxgl_qtbxDetail.DeleteFyFTMx(fykmGuid,billCode).value;
        if(returnValue==true)
        {   
            calYtYb();
            document.getElementById("btnRefresh").click();//刷新数据
    ...
    function onInputChange()
    {
        ...
        var returnValue=webBill_bxgl_qtbxDetail.UpdateFyftMxb(arr,billCode).value;
        
        if(returnValue==true)
        {
            $("#txtHjjeXx").val(webBill_bxgl_qtbxDetail.ToDoubleFormate(hj).value);toDaxieInfo();
        }
        else
        {
            alert("计算合计费用失败！");
    ...
    {
        var returnValue=webBill_bxgl_qtbxDetail.ChangeMoneyDept(mxGuid,document.getElementById("txt"+mxGuid).value).value;
    ...
    {
        var returnValue=webBill_bxgl_qtbxDetail.ChangeMoneyXm(mxGuid,document.getElementById("txt"+mxGuid).value).value;
    ...
    function changeSelect(obj,ftmxGuid)
    {
        ...
        var returnValue=webBill_bxgl_qtbxDetail.UpdateFtmxCbzx(ftmxGuid,val).value;
        if(returnValue=="")
        {
    ...
    function deleteJkmx()
    {
        ...
        var returnValue=webBill_bxgl_qtbxDetail.DeleteJkmxInfo(dkGuid).value;
        if(returnValue==true)
        {
            calYtYb();//计算应退应补
            document.getElementById("btnRefresh").click();//刷新数据
    ...
    {
        var billCode=document.getElementById("lblBillCode").innerHTML;
        var returnValue=webBill_bxgl_qtbxDetail.CaluateYtYb(billCode).value;
