...
public partial class webBill_bxgl_bxdprint_dz_fybx : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (modelMain != null)
            {
                //string deptcodename = modelMain.BillDept.ToString();
                ...
                //lblsqrq.Text = modelMain.Note1.ToString();
                //lblDept.Text = deptcodename;
                lblykrq.Text = modelMain[0].Note1;
                //lblsqr.Text = modelMain.BillUser.ToString();
            }
            ...
            if (modelybbxmx != null)
            {
                ...
                if (!string.IsNullOrEmpty(modelybbxmx.Sqlx))
                {
                    if (modelybbxmx.Sqlx == "fdl")
                    {
                        lblbxdlx.Text = "返点类";
                    }
                    else if (modelybbxmx.Sqlx == "jkfl")
                    {
                        lblbxdlx.Text = "讲课费类";
                    }
                    else
                ...
                lbl_fjs.Text = "&nbsp;" + modelybbxmx.Bxdjs.ToString() + "&nbsp;张";
            }
            //组建费用项目的明细html
            //项目明细
            ...
            StringBuilder sbFykm = new StringBuilder("<tr  style='height:40px'>");
            int flg = 0;
            for (int i = 1; i < dt.Rows.Count + 1; i++)
            {
                ...
                if (i > 1 && i % 3 == 0)
                {
                    sbFykm.Append("</tr><tr  style='height:40px'>");
                    flg = 0;
                }
                //如果转到最后了也没有正好凑够三列
            ...
            //            string strsql = @"select (select yskmmc from bill_yskm where yskmcode=item.fykm)as km,(select deptname from dbo.bill_departments where deptcode=main.billdept) as deptname,
            //                    * from Bill_Ybbxmxb m, Bill_Ybbxmxb_Fykm item,bill_main main
            //                    where m.billcode=item.billcode and main.billcode=m.billcode and main.billcode='" + billcode + "' ";
            ...
            for (int i = 1; i < splldt.Rows.Count + 1; i++)
            {
                ...
                string checktime = splldt.Rows[i - 1]["checkdate"].ToString();
                sbSpl.Append("<td><div>" + checkuser + "(" + splldt.Rows[i - 1]["sqzt"].ToString() + ")签字：" + checkuser + "</div><div>审批时间：" + checktime + "</div></td>");
                //每三列加一个tr标记
                if (i > 1 && i % 6 == 0)
                {
