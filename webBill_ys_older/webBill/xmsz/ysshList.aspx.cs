...
public partial class ysgl_ysshList : System.Web.UI.Page
{
    ...
    void BindDataGrid()
    {
        ...
        string billCodes = (new workFlowLibrary.workFlow()).getRightStepBills("xmys", Session["userGroup"].ToString().Trim(), Session["userCode"].ToString().Trim(),deptCodes);

        DataSet temp = server.GetDataSet("select (select '['+xmcode+']'+xmname from bill_xm where xmcode=billname2) as xmname,stepid as stepID_ID,(case stepid when '-1' then '未提交' when '0' then '审核退回' when 'begin' then '已提交' when 'end' then '审核通过' else (select steptext from bill_workFlowStep where bill_workFlowStep.flowID='xmys' and bill_workFlowStep.stepid=bill_main.stepid ) end) as stepID,billCode,billName as billNameCode,(select xmmc from bill_ysgc where gcbh=billName) as billName,(select username from bill_users where usercode=billuser) as billUser,billdate,billje from bill_main where billCode in (" + billCodes + ") and flowID='xmys'");
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
       ...
       if (count == 0)
        ...
        {
            (new workFlowLibrary.workFlow()).checkBills("xmys", billCode, Session["userCode"].ToString().Trim(), System.DateTime.Now.ToString(), shyj, true);
            this.BindDataGrid();
        }
    }
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        {
            (new workFlowLibrary.workFlow()).checkBills("xmys", billCode, Session["userCode"].ToString().Trim(), System.DateTime.Now.ToString(), shyj, false);
            this.BindDataGrid();
        }
    }
