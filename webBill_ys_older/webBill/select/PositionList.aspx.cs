...
public partial class PositionList : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from bill_dataDic where dicType=(select dicCode from bill_dataDic where dicName='职务类型')";
    ...
    protected void btn_select_Click(object sender, EventArgs e)
    {
        ...
        if (sel_count == 0)
        {
            ClientScript.RegisterStartupScript(this.GetType(),"", "<script>window.alert('请选择部门！');</script>");
        }
        else if (sel_count > 1)
        {
            ClientScript.RegisterStartupScript(this.GetType(), "", "<script>window.alert('只能选择一个部门！');</script>");
        }
        else
        {
